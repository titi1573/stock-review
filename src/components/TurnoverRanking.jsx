...
import { formatPct, numClass } from "../utils/format";
...
export default function TurnoverRanking({ data }) {
  ...
  const effectiveThreshold = threshold !== null ? threshold : Number(customVal) || 0;

  const filtered = useMemo(() => {
    const min = Math.max(10, effectiveThreshold);
    let list = data.items.filter(i => i.turnover >= min);
    list.sort((a, b) => {
  ...
  return (
    <section className="space-y-4">
      <h2 className="text-lg font-bold text-slate-100">成交额龙虎榜</h2>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/5 text-slate-500 text-xs">
                ...
                <th className="text-right px-4 py-3 font-medium cursor-pointer select-none hover:text-slate-300" onClick={() => handleSort("turnover")}>
                  成交额(亿)<SortIcon field="turnover" current={sort} />
                </th>
                <th className="text-right px-4 py-3 font-medium cursor-pointer select-none hover:text-slate-300" onClick={() => handleSort("change_pct")}>
                  涨跌幅<SortIcon field="change_pct" current={sort} />
                </th>
                <th className="text-left px-4 py-3 font-medium cursor-pointer select-none hover:text-slate-300" onClick={() => handleSort("sector")}>
            ...
            <tbody>
              {display.map((item, i) => (
                <tr key={item.code} className="border-b border-white/[0.02] hover:bg-white/[0.02]">
                  <td className={`px-4 py-2.5 text-xs font-mono tabular-nums ${i < 3 ? "text-amber-400" : "text-slate-600"}`}>{i + 1}</td>
                  ...
                  <td className={`text-right px-4 py-2.5 font-mono tabular-nums font-medium ${numClass(item.change_pct)}`}>{formatPct(item.change_pct)}</td>
        ...
        {display.map((item, i) => (
          <div key={item.code} className="glass-card p-3 flex items-center justify-between">
            ...
            <div className="text-right">
              <div className={`font-mono font-bold text-sm ${numClass(item.change_pct)}`}>{formatPct(item.change_pct)}</div>
      ...
      {totalEligible < 10 && <p className="text-xs text-slate-600 text-center">当前市场整体缩量，成交额集中度较低</p>}
      {totalEligible > 50 && !expanded && <p className="text-xs text-slate-600 text-center">满足条件共 {totalEligible} 只，仅展示前 20</p>}
      {totalEligible === 0 && <p className="text-xs text-slate-600 text-center">暂无满足条件的个股，可尝试降低阈值</p>}

      {totalEligible > 20 && (
        <button onClick={() => setExpanded(!expanded)} className="block mx-auto text-sm text-blue-400 hover:text-blue-300">
